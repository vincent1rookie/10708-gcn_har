...
class LOCGCN(nn.Module):
    ...
    def __init__(self,
        ...
        self.st_gcn_networks = nn.ModuleList((
            ...
            LOCGCNBlock(64, 64, kernel_size, 1, **kwargs),
            LOCGCNBlock(64, 64, kernel_size, 1, **kwargs),
            LOCGCNBlock(64, 64, kernel_size, 1, **kwargs),
            LOCGCNBlock(64, 128, kernel_size, 2, **kwargs),
            LOCGCNBlock(128, 128, kernel_size, 1, **kwargs),
            LOCGCNBlock(128, 128, kernel_size, 1, **kwargs),
            LOCGCNBlock(128, 256, kernel_size, 2, **kwargs),
            LOCGCNBlock(256, 256, kernel_size, 1, **kwargs),
            LOCGCNBlock(256, 256, kernel_size, 1, **kwargs),
        ))

        self.st_gcn_networks_feature = nn.ModuleList((
            LOCGCNBlock(
                32, 64, kernel_size, 1, residual=False, **kwargs0),
            LOCGCNBlock(64, 64, kernel_size, 1, **kwargs),
            LOCGCNBlock(64, 64, kernel_size, 1, **kwargs),
            LOCGCNBlock(64, 64, kernel_size, 1, **kwargs),
            LOCGCNBlock(64, 128, kernel_size, 2, **kwargs),
        ...
        if edge_importance_weighting:
            ...
            self.edge_importance_feature = nn.ParameterList([
                nn.Parameter(torch.ones(self.A.size()))
                for i in self.st_gcn_networks
            ])
        else:
            self.edge_importance = [1 for _ in self.st_gcn_networks]
            self.edge_importance_feature = [1 for _ in self.st_gcn_networks_feature]
    ...
    def forward(self, x):
        ...
        x_feature = x_feature.view(n * m, c, t, v)  # bsx2 3 300 25(17)
        # forward
        for gcn, importance in zip(self.st_gcn_networks, self.edge_importance):
            x_pos, _ = gcn(x_pos, self.A * importance)

        for gcn, importance in zip(self.st_gcn_networks_feature, self.edge_importance_feature):
            x_feature, _ = gcn(x_feature, self.A * importance)

        return x_pos+x_feature
